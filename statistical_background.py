...
# lista analizowanych urządzeń
DEVICE_IDS = ['10202', '10580', '10604', '10763', '10970', '11140', '11414',
    '11501', '11615', '11775', '11959', '11993', '12080', '12146',
    '12171', '12214', '12400', '12401', '12422', '12659', '12661',
    '1276', '12833', '13046', '13087', '1312', '13157', '13239',
    '13263', '1328', '13294', '13357', '1343', '13577', '13602',
    '13637', '13686', '13687', '1414', '1453', '1505', '4537', '4879',
    '6464', '7044', '7045', '7046', '7048', '7087', '7569', '7600',
    '7723', '7927', '7928', '8323', '9429', '9451', '9848'
]

# jeśli None -> licz globalnie dla wszystkich okien
# jeśli timestamp -> licz tylko dla tego jednego okna
TARGET_WINDOW: Optional[str] =  "2019-10-21 05:55:00+00:00"

# np.
# TARGET_WINDOW = "2019-10-21 05:55:00+00:00"
...
# =========================
def coincidence_background_per_window(
    df_all: pd.DataFrame,
    target_window: Optional[pd.Timestamp] = None,
) -> pd.DataFrame:
    rows = []
    ...
    for window_start, g in df_all.groupby("window_start", sort=True):
        if target_window is not None and pd.Timestamp(window_start) != pd.Timestamp(target_window):
            continue

        # bierzemy tylko urządzenia aktywne w tym oknie
        ...
        overactive_devices = g.loc[g["is_overactive_obs"], "device_id"].astype(str).tolist()
        ...
        rows.append(
            {
                ...
                "overactive_device_ids": ",".join(overactive_devices),
            }
        )
...
def summarize_background(df_all: pd.DataFrame, df_bg_win: pd.DataFrame) -> Dict[str, Any]:
    ...
    if df_bg_win.empty:
        return {
            ...
            "expected_single_overactive": float(df_all["q"].sum()),
            "expected_windows_ge_2_exact": 0.0,
            "expected_windows_ge_2_approx": 0.0,
            "observed_windows_ge_2": 0,
        }
...
def print_report(
    df_all: pd.DataFrame,
    df_bg_win: pd.DataFrame,
    summary: Dict[str, Any],
    target_window: Optional[pd.Timestamp] = None,
) -> None:
    print("\n" + "=" * 70)
    print("RAPORT ANALIZY TŁA STATYSTYCZNEGO KOINCYDENCJI")
    print("=" * 70)

    if target_window is None:
        print("Tryb analizy: GLOBALNY (wszystkie okna)")
    else:
        print(f"Tryb analizy: JEDNO WYBRANE OKNO -> {target_window}")
    ...
    print("-" * 70)
    print("TŁO STATYSTYCZNE:")
    print(
        "Oczekiwana liczba pojedynczych nadaktywnych urządzeń od samego tła "
        f"(po wszystkich analizowanych parach urządzenie-okno): {summary['expected_single_overactive']:.12g}"
    )
    print(
        "Oczekiwana liczba okien z koincydencją >=2 urządzeń od samego tła "
    ...
    if target_window is not None and not df_bg_win.empty:
        print("-" * 70)
        print("SZCZEGÓŁY WYBRANEGO OKNA:")
        row = df_bg_win.iloc[0]
        print(f"Znacznik czasu okna: {row['window_start']}")
        print(f"Liczba aktywnych urządzeń w tym oknie: {row['n_active_devices']}")
        print(f"Liczba nadaktywnych urządzeń w danych: {row['n_overactive_devices_obs']}")
        print(f"Czy wystąpiła koincydencja >=2 w danych: {bool(row['has_coincidence_obs'])}")
        print(f"Prawdopodobieństwo tła dla >=2 urządzeń w tym oknie (dokładnie): {row['p_background_ge_2_exact']:.12g}")
        print(f"Prawdopodobieństwo tła dla >=2 urządzeń w tym oknie (przybliżenie): {row['p_background_ge_2_approx']:.12g}")
        print(f"ID nadaktywnych urządzeń w tym oknie: {row['overactive_device_ids'] or '-'}")

        detail = df_all[df_all["window_start"] == row["window_start"]].copy()
        detail = detail[detail["on_time_seconds"] > 0].copy()
        detail = detail.sort_values(
            ["is_overactive_obs", "p_obs", "device_id"],
            ascending=[False, True, True]
        )

        print("\nAktywne urządzenia w wybranym oknie:")
        print(
            detail[
                [
                    "device_id",
...
def main() -> None:
    if not DEVICE_IDS:
        print("Lista DEVICE_IDS jest pusta.")
        return

    target_window = pd.Timestamp(TARGET_WINDOW) if TARGET_WINDOW else None
    ...
    df_bg_win = coincidence_background_per_window(
        df_all,
